...
app.delete('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
  ...
  var feedItemId = parseInt(req.params.feeditemid, 10);
  var commentIdx = parseInt(req.params.commentindex, 10);
  // Only a user can unlike their own like.
  if (fromUser === userId) {
    var feedItem = readDocument('feedItems', feedItemId);
    var comment = feedItem.comments[commentIdx];
...
app.post('/feeditem',
  ...
  * Must come after all of our defined routes (e.g. app.get, app.post, etc.)
  */
  ...
  //     res.send(reversed);
  //   } else {
  //     // POST did not contain a string (request syntax is malformed). Send an error code back!
  //     res.status(400).end();
  //   }
  // });
